...
### Descuento
**Atributos:** `id`, `tipo` (`PORCENTAJE`/`FIJO`), `valor`, `porcentaje`,
...
| `Carrito`          | ✅      | Bien modelado. `@JsonIgnore` en colecciones.                                         |
...
| `Foto`             | ✅      | Correcta.                                                                            |
| `Descuento`        | ⚠️      | Diseño de dos campos (`valor` + `porcentaje`) validado por el equipo. 🔍             |
...
| `CarritoRepository`         | ✅      | `findByUsuarioId`, `findByUsuarioIdAndEstado`.                       |
...
| `DescuentoRepository`       | ✅      | `findByEstado`.                                                      |
...
| `CarritoServiceImpl`         | ✅      | Bug fix stock en `realizarCompra()`. Validación producto ACTIVO en `agregarItem()`. `@Transactional`. `CarritoRequest`.              |
...
| `DescuentoServiceImpl`       | ✅      | `DescuentoRequest`. `@Transactional`. `estaVigente()`. `calcularDescuento()` con `RoundingMode.HALF_UP`.                             |
...
| `CarritoController`         | CRUD + items + total + vaciar + checkout                                                                                                               | ✅      | Requiere autenticación. Checkout devuelve `OrdenResponse`.                        |
| `OrdenController`           | `GET /`, `GET /{id}`, `DELETE /{id}`, `POST /{id}/confirmar`, `POST /{id}/cancelar`, `GET /{id}/monto-final`, `GET /{id}/items`, `GET /usuario/{id}` | ✅      | Requiere autenticación.                                                           |
...
| `DescuentoController`       | `GET /`, `GET /{id}`, `POST /`, `PUT /{id}`, `DELETE /{id}`, `GET /{id}/vigente`, `GET /{id}/calcular`                                                | ✅      | Solo ADMIN.                                                                       |
...
| `DescuentoRequest`        | Input crear/actualizar descuento         | ✅      |
| `DescuentoResponse`       | Output descuento                         | ✅      |
| `CarritoRequest`          | Input crear/actualizar carrito           | ✅      |
| `CarritoResponse`         | Output carrito con items embebidos       | ✅      |
...
4. **Cobertura de tests** — actualmente hay 1 test de integración. Faltan tests para:
   ...
   descuentos, transiciones de estado de orden, y endpoints protegidos por rol.

5. **Features pendientes (no críticas):**
   - Paginación en endpoints de colecciones (`Page<T>`).
   - Historial de carritos por usuario.
   - Lógica de expiración automática de descuentos (scheduled task).
